 ...
 * 全部都是 chevron——面板列右邊的 `›`、開空間的「產生名單 ›」——只有返回
 * 曾經是另一種畫法（一支實心箭），於是它在同一個畫面上讀起來像另一個系統的
 * 東西。（2026-09 之後「調整清單」用的也是這一顆。）
 *
...
/*
 * `IconChevronDown` 2026-09 移除，跟著設定面板的就地展開一起走。
 * 這個 app 現在只有兩個方向的箭頭：`›` 進去（IconChevronRight）、
 * `‹` 回來（IconBack）。**沒有第三種，因為沒有第三種去處。**
 */
/*
 * `IconChevronUp` 2026-09 移除：它只有一個使用者（開空間的「調整清單 ⌃」），
 * 而那一顆換成了 `IconBack`——那件事是「回上一步」，不是「收起來」。
 * 收合狀態的 `⌃` 一直都是 `.chevron.is-open` 把 `⌄` 轉 180° 轉出來的，
 * 不需要第二個圖示。
 */
// 選單列右邊那顆：「這一列會開一張子畫面」。沒有它，一份選單裡「開一頁」與
// 「當場做一件事」長得一模一樣（見 .sheet-item .go）。
...
// 分享那三列各有自己的圖示：代碼（#）、連結（鏈）、二維碼（三個定位角）。
export const IconHash = ({ size = 20, ...r }: P) => (
  <svg {...base(size)} {...r}>
...
export const IconQr = ({ size = 20, ...r }: P) => (
  <svg {...base(size)} {...r}>
    <rect x="3" y="3" width="7" height="7" rx="1" /><rect x="14" y="3" width="7" height="7" rx="1" />
    <rect x="3" y="14" width="7" height="7" rx="1" />
    <line x1="14" y1="14" x2="14" y2="18" /><line x1="18" y1="14" x2="21" y2="14" />

...
- [`docs/design-review-2026-09-shape.md`](docs/design-review-2026-09-shape.md) — 形狀與類型重整：38 條形狀規則裡有 20 條是同一個圓角，於是形狀完全不參與溝通；規範說 `--r-3` 是「卡片、面板、對話框」而實作裡的卡片從來都不是它；規範訂了兩種表面、實作有三種而第三種沒有名字。訂下兩條互不干涉的軸（**表面說種類、圓角說尺寸**），並把四條規則寫成測試
- [`docs/design-review-2026-09-ios.md`](docs/design-review-2026-09-ios.md) — 以 Apple HIG 為標竿的評估：換一把外部的尺之後量到的東西（完全不支援 Dynamic Type、系統返回手勢沒接上所以右滑會直接退出空間、iOS 會在七天後清掉單機模式的名單、品牌規範宣稱了一個 iOS 不提供的能力）。**平台整合的缺陷不會出現在截圖裡**
...
不知道從哪開始的話，開空間那一頁的右上角有「填入範例」：按一下會把活動名稱與上面這份名單一起填好，解析結果直接列在下面，等於先看一遍完整的空間長什麼樣；看完按「清除範例」就收乾淨了。你自己打過的字它不會碰——已經填了活動名稱再按範例，只會補上名單。

**但你不用先整理成這樣。** LINE 接龍長什麼樣就貼什麼樣：編號是 `1.` `2、` `3)` `4：` 還是全形「４．」都認得，`-` `•` `*` 這些項目符號、沒有編號的光名字、全形數字與標點、括號備註也一樣。

分組是唯一嚴格的那一條：只認 `#`（全形 `＃` 與 `##` 也可以）。一條規則，看著自己打的字就知道哪幾行會變成車次。`#1 王小明` 不算——井字號後面接編號加名字，那是人。

這幾條規則 app 裡也有：開空間那一頁頂欄，標題「創建空間」右邊那顆 **?**，每一條都配一個例子。
備註寫「請假 / 不去 / 取消」的人會**直接標成請假**，不會混進未到清單被打電話。

**名字到第一個「空白＋數字」為止，後面整段原文進備註。** 電話、匯款帳號、身分證、座位號一律照抄，解析器不去判斷那串數字是什麼——它猜錯的時候，「匯款 700-1234567」會變成一支撥出去是空號的假電話，而畫面上沒有一個字說得出為什麼。切在空白而不是第一個數字，是為了讓「Alice Chen 0912345678」切在號碼前面而不是切在 `Chen` 前面。

要撥號的話，點那個人的 **⋯ → 成員面板**：備註裡「`0` 或 `+` 開頭、8～15 碼」的號碼會做成撥號鍵，打完那一列會留下「已撥 15:32」。國際號碼（`+65 9123 4567`）照樣撥得出去——備註存的是原文，`tel:` 自己認得。
...
docs/
  ...
  design-review-2026-09-shape.md  形狀與類型：表面說種類、圓角說尺寸
  design-review-2026-09-ios.md    以 Apple HIG 為標竿：平台整合與清單密度
```

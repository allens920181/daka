...
不知道從哪開始的話，貼上名單那一欄的右上角有「填入範例」，按一下就把上面這段填進去，解析結果會直接列在下面；看完按「清除範例」就收乾淨了。

範例第一行的活動標題是故意留著的：真實的接龍最上面就是這一行，貼進來會被當成一個人。在預覽裡按那一列的 ✕ 就沒了——這是你自己的名單也會遇到的第一件事，範例先讓你遇到一次。
